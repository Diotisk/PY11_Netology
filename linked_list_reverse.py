...
my_list = LinkedList(values=[1, 2, 3, 4, 5])
print(list(my_list))
start = time.time()
...
# сложность алгоритма и потребление памяти - O(n) за счет итерирования списка
# в случае с массивом, если реализовывать метод reverse через поиск и изменение индекса, сложность и потребление памяти
# будет O(1), потому что массив не надо будет итерировать для поиска значений, его последовательность неизменяема и
# не зависима от соседних значений
